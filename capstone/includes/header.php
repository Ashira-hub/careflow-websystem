...
try {
  ...
  if ($isLabPath && !$isLabDashboard && $method === 'GET') {
    ...
    if (!isset($_SESSION['lab_activity']) || !is_array($_SESSION['lab_activity'])) { $_SESSION['lab_activity'] = []; }
  ...
  if ($isPharmPath && !$isPharmDashboard && $method === 'GET') {
    ...
    if (!isset($_SESSION['pharmacy_activity']) || !is_array($_SESSION['pharmacy_activity'])) { $_SESSION['pharmacy_activity'] = []; }
...
<head>
  ...
  <style>
    ...
    .dark .card, .dark section.card, .dark .stat-card { background:#111827 !important; border-color:#1f2937 !important; color:#e5e7eb !important; }
    .dark .sidebar, .dark .sidepanel { background:#0b1220; border-color:#1f2937; }
    .dark .nav-list a { color:#e5e7eb; }
    .dark .nav-list a.active { color:#10b981; }
    .dark .dashboard-title, .dark h1, .dark h2, .dark h3, .dark h4 { color:#e5e7eb; }
    .dark .muted, .dark .muted-small { color:#9ca3af !important; }
    .dark input, .dark select, .dark textarea { background:#0f172a !important; border-color:#1f2937 !important; color:#e5e7eb !important; }
    .dark table { color:#e5e7eb; }
    .dark table thead tr { background:#0f172a; }
    .dark table tr { border-color:#1f2937; }
    .dark .btn.btn-outline { border-color:#374151; color:#e5e7eb; }
    .dark .btn.btn-outline:hover { background:#111827; }
    .dark .alert { background:#0f172a !important; border-color:#1f2937 !important; color:#e5e7eb !important; }
  </style>
  <script>
    (function(){
      try{
    ...
    (function(){
      ...
      <?php if (!empty($_SESSION['flash_welcome'])): ?>
        ...
        document.addEventListener('DOMContentLoaded', function(){
          ...
          setTimeout(function(){ toast.style.opacity='0'; toast.style.transform='translateY(-8px)'; setTimeout(function(){ if(toast.parentNode){ toast.parentNode.removeChild(toast); } }, 300); }, 2800);
        });
      <?php $_SESSION['flash_welcome'] = null; unset($_SESSION['flash_welcome']); endif; ?>

...
if ($method === 'POST') {
  ...
  try {
    ...
    if ($hasNurseEmail) {
      $cols[] = 'nurse_email';
      $vals[] = ':nurse_email';
      $params[':nurse_email'] = $nurseEmail;
    }
    if ($colDate) {
      $cols[] = '"' . $colDate . '"';
      $vals[] = 'CAST(:date AS date)';
      $params[':date'] = $date;
    }
    if ($colShift) {
      $cols[] = $colShift;
    ...
    if ($colWard) {
      ...
      $notes = trim($notes . ' | Station: ' . $ward);
    }
    if ($colStart) {
      $cols[] = '"' . $colStart . '"';
      $vals[] = 'CAST(:start_time AS time)';
      $params[':start_time'] = $startTime;
    } else {
      $notes = trim($notes . ' | Start: ' . $startTime);
    }
    if ($colEnd) {
      $cols[] = '"' . $colEnd . '"';
      $vals[] = 'CAST(:end_time AS time)';
      $params[':end_time'] = $endTime;
    } else {
      $notes = trim($notes . ' | End: ' . $endTime);
    }
    if ($colNotes) {
      $cols[] = $colNotes;
      $vals[] = ':notes';
    ...
    if ($hasCreatedBy && $createdByUserId) {
      $cols[] = 'created_by_user_id';
      $vals[] = ':created_by_user_id';
    ...
    if (empty($cols)) {
      http_response_code(500);
      echo json_encode(['error' => 'Table schedules has no compatible columns']);
      exit;
    }
...
if ($method === 'PUT') {
  ...
  try {
    ...
    $sets = [];
    $params = [':id' => $id];
    if (array_key_exists('status', $input) && $colStatus) {
      $sets[] = '"' . $colStatus . '" = :status';
      $params[':status'] = strtolower((string)$input['status']);
    }
    if (array_key_exists('notes', $input) && $colNotes) {
      $sets[] = '"' . $colNotes . '" = :notes';
      $params[':notes'] = (string)$input['notes'];
    }
    if ($hasUpdatedAt) {
      $sets[] = 'updated_at = now()';
    }

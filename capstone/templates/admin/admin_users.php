    ...
    if ($hasUserId) {
      ...
      $sql = 'INSERT INTO doctor (' . implode(',', $insCols) . ') ' .
        ...
        implode(', ', array_map(function ($c) {
          return $c . " = EXCLUDED." . $c;
        }, array_filter($insCols, function ($c) {
          return $c !== 'user_id' && $c !== 'created_at';
        })));
      $pdo->exec($sql);
    } else {
...
try {
  ...
  if (in_array('full_name', $nurseCols, true)) {
    $insCols[] = 'full_name';
    $selCols[] = 'u.full_name';
  }
  if (in_array('email', $nurseCols, true)) {
    $insCols[] = 'email';
    $selCols[] = 'u.email';
  }
  if ($nHasRole) {
...
try {
  ...
  if (in_array('full_name', $phCols, true)) {
    $insCols[] = 'full_name';
    $selCols[] = 'u.full_name';
  }
  if (in_array('email', $phCols, true)) {
    $insCols[] = 'email';
    $selCols[] = 'u.email';
  }
  if ($pHasRole) {
...
try {
  ...
  if (in_array('full_name', $supCols, true)) {
    $insCols[] = 'full_name';
    $selCols[] = 'u.full_name';
  }
  if (in_array('email', $supCols, true)) {
    $insCols[] = 'email';
    $selCols[] = 'u.email';
  }
  if ($sHasRole) {
...
try {
  ...
  if (in_array('full_name', $labCols, true)) {
    $insCols[] = 'full_name';
    $selCols[] = 'u.full_name';
  }
  if (in_array('email', $labCols, true)) {
    $insCols[] = 'email';
    $selCols[] = 'u.email';
  }
  if ($lHasRole) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($action === 'create') {
    ...
    if (!in_array($roleNorm, $validRoles, true)) {
      $roleNorm = $roleLower;
    }
    ...
    $vErrors = [];
    if ($fullName === '') {
      $vErrors[] = 'Full name is required';
    }
    if ($email === '' || !filter_var($email, FILTER_VALIDATE_EMAIL)) {
      $vErrors[] = 'Valid email is required';
    }
    if ($password === '' || strlen($password) < 6) {
      $vErrors[] = 'Password must be at least 6 characters';
    }
    if ($roleNorm === '' || !in_array($roleNorm, $validRoles, true)) {
      $vErrors[] = 'Valid role is required';
    }
    if (!empty($vErrors)) {
      if ($isAjax) {
...
<script>
  (function() {
    ...
    if (backdropEdit) {
      backdropEdit.addEventListener('click', closeEditModal);
    }
    ...
    if (pw && pwToggle) {
      pwToggle.addEventListener('click', function() {
        if (pw.type === 'password') {
          pw.type = 'text';
          if (pwIcon) {
            pwIcon.src = '/capstone/assets/img/hidden.png';
            pwIcon.alt = 'Hide';
          }
          ...
          pw.type = 'password';
          if (pwIcon) {
            pwIcon.src = '/capstone/assets/img/eye.png';
            pwIcon.alt = 'Show';
          }

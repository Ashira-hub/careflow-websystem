...
try {
  ...
  $set = [];
  foreach ($patientsA as $p) {
    $k = strtolower(trim((string)$p));
    if ($k !== '') $set[$k] = 1;
  }
  foreach ($patientsB as $p) {
    $k = strtolower(trim((string)$p));
    if ($k !== '') $set[$k] = 1;
  }
...
try {
  ...
  $cntAppt = $cntAppt ?? 0;
  $cntRx = $cntRx ?? 0;
  $cntPatients = $cntPatients ?? 0;
  $cntReports = $cntReports ?? 0;
}
?>
<?php
...
try {
  ...
  usort($recentItems, function ($x, $y) {
    ...
    <section id="home" class="dashboard-hero doctor-dashboard-hero">
      ...
      <div class="stat-cards">
        <div class="card">
          ...
          <div class="stat-value" id="dbAppt"><?php echo (int)$countAppt; ?></div>
        </div>
        <div class="card">
          <h4>Prescriptions</h4>
          <div class="muted-small">Total</div>
          <div class="stat-value" id="dbRx"><?php echo (int)$countRx; ?></div>
        </div>
        <div class="card">
          <h4>Patient Records</h4>
          <div class="muted-small">Total</div>
          <div class="stat-value" id="dbPatients"><?php echo (int)$uniquePatients; ?></div>
        </div>
        <div class="card">
          <h4>Reports</h4>
          <div class="muted-small">Available</div>
          <div class="stat-value" id="dbUnread"><?php echo (int)$cntReports; ?></div>
        </div>
      </div>
...
<script>
  ...
  window.DOCTOR_RECENT = <?php echo json_encode(array_map(function ($it) {
                            return ['title' => $it['title'], 'meta' => $it['meta'], 'body' => $it['body']];
                          }, $recentItems), JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES); ?>;
</script>
<script>
  (function() {
    ...
    function renderList(container, items, emptyText) {
      ...
      if (!items || items.length === 0) {
        container.innerHTML = '<div class="muted">' + escapeHtml(emptyText || 'No data') + '</div>';
        return;
      }
      var html = '<ul class="plain-list">' + items.map(function(x) {
        return '<li>' + x + '</li>';
      }).join('') + '</ul>';

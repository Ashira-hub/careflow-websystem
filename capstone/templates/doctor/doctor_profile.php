  ...
  try {
    ...
    if ($existsStmt->fetch()) {
      ...
      $ins->execute([
        ':id' => $userId,
        ':full_name' => $fullName,
        ':role' => (string)($_SESSION['user']['role'] ?? 'doctor'),
        ':email' => $email,
        ':phone' => $phone,
        ':address' => $address,
        ':gender' => $gender,
    ...
    <div class="sidepanel">
      <nav>
        <ul class="nav-list">
          ...
          <li><a href="/capstone/templates/doctor/doctor_records.php"><img src="/capstone/assets/img/medical-file.png" alt="Patient Record" style="width:18px;height:18px;object-fit:contain;"> Patient Records</a></li>
  ...
  <div>
    ...
    <div class="profile-header" style="background:linear-gradient(135deg,#ffffff,#f8fafc);border:1px solid #e2e8f0;border-radius:20px;padding:32px;box-shadow:0 8px 24px rgba(0,0,0,0.08);margin-bottom:24px;">
      <div class="profile-left" style="display:flex;align-items:center;gap:24px;">
        ...
        <div>
          <h2 class="profile-name" style="margin:0 0 8px;font-size:1.8rem;font-weight:700;color:#0f172a;"><?php echo htmlspecialchars($full ?: 'Doctor'); ?></h2>

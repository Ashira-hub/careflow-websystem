    ...
    <section class="card">
      ...
      <table style="width:100%;border-collapse:collapse;">
        ...
        <tbody id="drxBodyRows">
          <tr>
            <td colspan="5" class="muted" style="text-align:center;padding:20px;">No records yet.</td>
          </tr>
        </tbody>
      </table>
    </section>
  ...
  (function() {
    ...
    function parseFields(n) {
      ...
      body.split('|').forEach(function(part) {
        part = part.trim();
        if (part.toLowerCase().startsWith('patient:')) {
          patient = part.split(':')[1]?.trim() || '';
        }
        if (part.toLowerCase().startsWith('medicine:')) {
          med = part.split(':')[1]?.trim() || '';
        }
    ...
    function buildRows() {
      ...
      cache.forEach(function(n) {
        ...
        rows.push({
          ...
          time: n.time || '',
          patient: f.patient || '',
          medication: f.medicine || '',
          dosage: f.dosage || '',
          status: statusText(n)
        });
      });
      appts.forEach(function(a) {
        rows.push(formatApptRow(a));
      });
      return rows;
    }
    ...
    function renderGrouped(rows) {
      tbody.innerHTML = '';
      if (rows.length === 0) {
        tbody.innerHTML = '<tr><td colspan="5" class="muted" style="text-align:center;">No records yet.</td></tr>';
        return;
      }
      var groups = {};
      ...
      var patientNames = Object.keys(groups).sort(function(a, b) {
        return a.localeCompare(b);
      });
      patientNames.forEach(function(pName) {
        var items = groups[pName].slice().sort(function(a, b) {
    ...
    function openModal(patientName, patientRecords) {
      ...
      if (patientRecords.length === 0) {
        ...
        var apptItems = patientRecords.filter(function(r) {
          return r.type === 'appointment';
        });
        var rxItems = patientRecords.filter(function(r) {
    ...
    if (fPatient) {
      fPatient.addEventListener('input', function() {
        applyAndRender();
      });
    }
    if (fDate) {

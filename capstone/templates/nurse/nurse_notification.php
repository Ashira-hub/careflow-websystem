<?php $page='Nurse Notifications'; include __DIR__.'/../../includes/header.php'; ?>
  ...
  <div class="content" style="max-width:1200px;margin:0 auto;">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;">
      ...
      <div style="display:flex;gap:8px;align-items:center;">
        <button class="btn btn-outline" id="nrsNFClear" title="Clear all">Clear all</button>
      </div>
    </div>

    <div id="nrsNFEmpty" class="muted" style="display:none;padding:12px 0;">You're all caught up. No notifications.</div>

    <div id="nrsNFList" style="background:#fff;border:1px solid #e5e7eb;border-radius:10px;overflow:hidden;"></div>
  </div>
</div>
...
(function(){
  ...
  var list = document.getElementById('nrsNFList');
  var empty = document.getElementById('nrsNFEmpty');
  var btnClear = document.getElementById('nrsNFClear');
  ...
  async function load(){
    try{
      var res = await fetch('/capstone/notifications/pharmacy.php?role=nurse', { cache: 'no-store' });
  ...
  function closeModal(){ if(modal){ modal.style.display='none'; document.body.style.overflow=''; currentNotificationId=null; } }

  // Clear all
  if(btnClear){
    btnClear.addEventListener('click', async function(){
      try{
        var res = await fetch('/capstone/notifications/pharmacy.php?role=nurse', { method:'DELETE' });
  ...
  // Row actions (delete or open modal)
  if(list){
    list.addEventListener('click', function(e){
      ...
      if(btn && btn.getAttribute('data-action') === 'delete'){
        ...
        (async function(){
          try{
            await fetch('/capstone/notifications/pharmacy.php?role=nurse&id='+encodeURIComponent(id), { method:'DELETE', cache:'no-store' });
          }catch(_){ /* ignore network errors, still update UI */ }
    ...
    // Hover effects
    list.addEventListener('mouseenter', function(e){ var r=e.target.closest('.nf-item'); if(r) r.style.backgroundColor='#f8fafc'; });
    list.addEventListener('mouseleave', function(e){ var r=e.target.closest('.nf-item'); if(r){ var id=parseInt(r.getAttribute('data-id'),10); var n=notifications.find(function(x){return x.id===id;}); r.style.backgroundColor = n && n.read ? '#fafafa' : ''; } });
  }
    ...
    deleteModalBtn.addEventListener('click', function(){
      if(currentNotificationId){
        ...
        if(idx !== -1){
          ...
          (async function(){
            try{
              await fetch('/capstone/notifications/pharmacy.php?role=nurse&id='+encodeURIComponent(currentNotificationId), { method:'DELETE', cache:'no-store' });
            }catch(_){ }
            notifications.splice(idx,1);
            render();
          })();

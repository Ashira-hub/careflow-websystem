...
<script>
  (function() {
    ...
    async function load() {
      try {
        var res = await fetch('/capstone/notifications/pharmacy.php?role=nurse', {
          cache: 'no-store'
        });
    ...
    if (list) {
      list.addEventListener('click', function(e) {
        ...
        if (btn && btn.getAttribute('data-action') === 'delete') {
          ...
          (async function() {
            try {
              await fetch('/capstone/notifications/pharmacy.php?role=nurse&id=' + encodeURIComponent(id), {
                method: 'DELETE',
                cache: 'no-store'
              });
            } catch (_) {
      ...
      list.addEventListener('mouseenter', function(e) {
        var r = e.target.closest('.nf-item');
        if (r) r.style.backgroundColor = '#f8fafc';
      });
      list.addEventListener('mouseleave', function(e) {
        ...
        if (r) {
          ...
          r.style.backgroundColor = n && n.read ? '#fafafa' : '';
        }
      });
    }
    ...
    if (acceptModalBtn) {
      acceptModalBtn.addEventListener('click', async function() {
        ...
        if (!m) {
          alert('No ScheduleID found in this notification.');
          return;
        }
      ...
      deleteModalBtn.addEventListener('click', function() {
        if (currentNotificationId) {
          ...
          if (idx !== -1) {
            ...
            (async function() {
              try {
                await fetch('/capstone/notifications/pharmacy.php?role=nurse&id=' + encodeURIComponent(currentNotificationId), {
                  method: 'DELETE',
                  cache: 'no-store'
                });
              } catch (_) {}

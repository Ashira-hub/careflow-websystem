...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  if (empty($_SESSION['user']['id'])) {
    ...
    header('Location: /capstone/templates/nurse/nurse_profile.php');
    exit;
  }
  ...
  if (!empty($_FILES['avatar']) && isset($_FILES['avatar']['tmp_name']) && is_uploaded_file($_FILES['avatar']['tmp_name'])) {
    try {
      ...
      $destDir = $uploadRoot !== false ? $uploadRoot . DIRECTORY_SEPARATOR . 'uploads' . DIRECTORY_SEPARATOR . 'avatars' : __DIR__ . '/../../uploads/avatars';
  ...
  try {
    ...
    // Align to Railway profile schema (id-based)
    $exists = $pdo->prepare('SELECT 1 FROM profile WHERE id = :id LIMIT 1');
    ...
    if ($exists->fetch()) {
      ...
      $ins->execute([
        ':id' => $userId,
        ':full_name' => $fullName,
        ':role' => (string)($_SESSION['user']['role'] ?? 'nurse'),
        ':email' => $email,
        ':phone' => $phone,
        ':address' => $address,
        ':gender' => $gender,
...
<div class="layout-sidebar full-bleed" style="padding: 24px 20px;">
  ...
  <div>
    <?php
      ...
      $roleName = isset($_SESSION['user']['role']) ? ucwords(str_replace('_',' ', $_SESSION['user']['role'])) : 'Nurse';
    ?>
    <div class="profile-header" style="background:linear-gradient(135deg,#ffffff,#f8fafc);border:1px solid #e2e8f0;border-radius:20px;padding:32px;box-shadow:0 8px 24px rgba(0,0,0,0.08);margin-bottom:24px;">
      <div class="profile-left" style="display:flex;align-items:center;gap:24px;">
        <div class="avatar" style="width:100px;height:100px;border-radius:50%;background:linear-gradient(135deg,#0a5d39,#10b981);display:flex;align-items:center;justify-content:center;box-shadow:0 8px 24px rgba(10,93,57,0.3);border:4px solid #fff;position:relative;overflow:hidden;">
          ...
          <?php else: ?>
            <img src="/capstone/assets/img/nurse.png" alt="Avatar" style="width:50px;height:50px;object-fit:contain;filter:brightness(0) invert(1);" />
          <?php endif; ?>
        </div>
        <div>
          <h2 class="profile-name" style="margin:0 0 8px;font-size:1.8rem;font-weight:700;color:#0f172a;"><?php echo htmlspecialchars($full ?: 'Nurse'); ?></h2>
    ...
    <h4 class="profile-section-title" style="color:#0a5d39;font-size:1.2rem;font-weight:700;margin-bottom:20px;padding-bottom:8px;border-bottom:2px solid #e2e8f0;">Nurse Information</h4>
      ...
      <div style="padding:28px;max-height:70vh;overflow-y:auto;scrollbar-width:thin;scrollbar-color:#0a5d39 #f1f5f9;" class="scrollable-content">
        ...
        <div style="text-align:center;margin-bottom:32px;">
          <div style="position:relative;display:inline-block;">
            <div id="avatarPreview" style="width:120px;height:120px;border-radius:50%;background:linear-gradient(135deg,#0a5d39,#10b981);display:flex;align-items:center;justify-content:center;margin:0 auto 16px;box-shadow:0 8px 24px rgba(10,93,57,0.3);border:4px solid #fff;">
              ...
              <?php else: ?>
                <img src="/capstone/assets/img/nurse.png" alt="Avatar" style="width:60px;height:60px;object-fit:contain;filter:brightness(0) invert(1);" />
              <?php endif; ?>
            </div>

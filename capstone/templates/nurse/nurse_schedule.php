...
<div id="requestModal" style="display:none;position:fixed;inset:0;z-index:1000;">
  ...
  <script>
    (function() {
      ...
      function composeKey(it) {
        return (it && it.id) ? ('id:' + it.id) : ('k:' + [it && it.date || '', it && it.shift || '', it && it.start_time || '', it && it.end_time || '', (it && (it.ward || it.station)) || ''].join('|'));
      }
      ...
      function renderNsPage() {
        ...
        nsAllItems.slice(start, end).forEach(function(it) {
          appendClientRequest(it, true);
        });
        if (nsPager) nsPager.style.display = 'flex';
        if (nsPageIndicator) {
          nsPageIndicator.textContent = 'Page ' + nsCurrentPage + ' / ' + totalPages;
        }
        if (nsPrevPage) {
          nsPrevPage.disabled = (nsCurrentPage <= 1);
        }
        if (nsNextPage) {
          nsNextPage.disabled = (nsCurrentPage >= totalPages);
        }
      }
      ...
      function appendClientRequest(it, fromRender) {
        ...
        if (fromRender) {
          ...
          shiftList.insertBefore(card, shiftList.firstChild);
        }
      }
      ...
      async function loadRequests() {
        ...
        if (shiftEmpty) {
          shiftEmpty.textContent = 'Loading...';
          shiftEmpty.style.display = '';
        }
      ...
      if (form) {
        form.addEventListener('submit', async function(e) {
          ...
          if (!shift || !date || !station || !startTime || !endTime) {
          ...
          if (cleanNotes) {
            bodyParts.push('Notes: ' + cleanNotes);
          }
          try {
            var payload = {
              nurse: nurseName,
            ...
            if (!storeRes.ok) {
              var storeText = await storeRes.text().catch(function() {
                return '';
              });
              throw new Error(storeText || 'Failed to save request');
            }

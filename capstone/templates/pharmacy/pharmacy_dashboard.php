...
foreach ($inventoryItems as $row) {
  ...
  if ($expTs && $expTs >= $now && $expTs <= $soonThreshold) {
    $expiringSoonCount++;
  }
  $createdTs = $row['created_at'] ? strtotime((string)$row['created_at']) : null;
  if ($createdTs && (!$latestInventoryTs || $createdTs > $latestInventoryTs)) {
    $latestInventoryTs = $createdTs;
  }
}
...
<script>
  ...
  window.PHARMACY_RECENT = <?php echo json_encode(array_map(function ($it) {
                              return ['title' => $it['title'], 'meta' => $it['meta'], 'body' => $it['body']];
                            }, $recentPharmacyItems), JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES); ?>;
</script>

<?php $page='Pharmacy Notifications'; include __DIR__.'/../../includes/header.php'; ?>
...
<div class="layout-sidebar full-bleed" style="padding: 24px 20px;">
  <aside class="sidebar">
    <div class="sidepanel">
      <nav>
        <ul class="nav-list">
          <li><a href="#home"><img src="/capstone/assets/img/home (2).png" alt="Home" style="width:18px;height:18px;object-fit:contain;"> Home</a></li>
...
(function(){
  ...
  async function load(){
    try{
      ...
      if(list) list.innerHTML = '<div class="muted" style="padding:12px;">'+escapeHtml(err.message)+'</div>';
  ...
  if(btnClearAll){
    btnClearAll.addEventListener('click', async function(){
      try{
        var res = await fetch('/capstone/notifications/pharmacy.php?role=pharmacy', { method:'DELETE' });

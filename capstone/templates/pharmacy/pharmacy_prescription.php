...
<style>
  ...
  .rx-form-fields .form-field {
    display: flex;
    flex-direction: column;
    gap: 6px;
  }

  .rx-form-fields label {
    font-size: .78rem;
    font-weight: 600;
    color: #475569;
    ...
    <script>
      (function() {
        ...
        var STATUS_META = {
          ...
          'dispensed': {
            text: 'Dispensed',
            className: 'badge badge-dispensed'
          },
          'acknowledged': {
            text: 'Acknowledged',
            className: 'badge badge-acknowledged'
          },
          'done': {
            text: 'Done',
        ...
        function openModal(item) {
          ...
          if (fPatient) {
            fPatient.value = fields.patient || '';
          }
          if (fMedicine) {
            fMedicine.value = fields.medicine || '';
          }
          if (fQty) {
            fQty.value = fields.qty || '';
          }
          if (fNotes) {
            fNotes.value = fields.notes || item.body || '';
          }
        ...
        if (bCloseTop) {
          bCloseTop.addEventListener('click', closeModal);
        }
        ...
        if (bDispense) {
          bDispense.addEventListener('click', function() {
            handleStatus('dispensed');
          });
        }

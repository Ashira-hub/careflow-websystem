...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  if (empty($_SESSION['user']['id'])) {
    ...
    header('Location: /capstone/templates/pharmacy/pharmacy_profile.php');
    exit;
  }
  ...
  try {
    ...
    // Keep users table in sync (including extended columns if present)
    // We optimistically update phone/gender/birthdate/address/avatar_uri; if a column is missing, DB will raise once and you can remove it.
    ...
    if ($existsStmt->fetch()) {
      ...
      $ins->execute([
        ':id'        => $userId,
        ':full_name' => $fullName,
        ':role'      => (string)($_SESSION['user']['role'] ?? 'pharmacist'),
        ':email'     => $email,
        ':phone'     => $phone,
        ':address'   => $address,
        ':gender'    => $genderParam,
...
if (!empty($_SESSION['user']['id'])) {
  try {
    ...
    // Also load extended fields from users table and prefer them when present
    ...
    if ($userRow) {
      ...
      // Prefer users table for these fields when available
      if (!empty($userRow['phone'])) {
        $profile['phone']      = $userRow['phone'];
      }
      if (!empty($userRow['address'])) {
        $profile['address']    = $userRow['address'];
      }
      if (!empty($userRow['birthdate'])) {
        $profile['birthdate']  = $userRow['birthdate'];
      }
      if (!empty($userRow['gender'])) {
        $profile['gender']     = $userRow['gender'];
      }
      if (!empty($userRow['avatar_uri'])) {
        $profile['avatar_uri'] = $userRow['avatar_uri'];
      }
    }
  } catch (Throwable $e) {
...
<div class="layout-sidebar full-bleed" style="padding: 24px 20px;">
  ...
  <div>
    ...
    <div class="profile-header" style="background:linear-gradient(135deg,#ffffff,#f8fafc);border:1px solid #e2e8f0;border-radius:20px;padding:32px;box-shadow:0 8px 24px rgba(0,0,0,0.08);margin-bottom:24px;">
      <div class="profile-left" style="display:flex;align-items:center;gap:24px;">
        <div class="avatar" style="width:100px;height:100px;border-radius:50%;background:linear-gradient(135deg,#0a5d39,#10b981);display:flex;align-items:center;justify-content:center;box-shadow:0 8px 24px rgba(10,93,57,0.3);border:4px solid #fff;position:relative;overflow:hidden;">
          ...
          <?php else: ?>
            <img src="/capstone/assets/img/drug.png" alt="Avatar" style="width:50px;height:50px;object-fit:contain;filter:brightness(0) invert(1);" />
          <?php endif; ?>
        ...
        <div>
          <h2 class="profile-name" style="margin:0 0 8px;font-size:1.8rem;font-weight:700;color:#0f172a;"><?php echo htmlspecialchars($full ?: 'Pharmacist'); ?></h2>
    ...
    <h4 class="profile-section-title" style="color:#0a5d39;font-size:1.2rem;font-weight:700;margin-bottom:20px;padding-bottom:8px;border-bottom:2px solid #e2e8f0;">Pharmacist Information</h4>
...
<div id="profileModal" style="display:none;position:fixed;inset:0;z-index:1000;overflow-y:auto;">
  ...
  <div role="dialog" aria-modal="true" style="position:relative;max-width:600px;margin:2vh auto;background:#fff;border-radius:20px;box-shadow:0 25px 50px rgba(0,0,0,0.25);overflow:hidden;min-height:fit-content;">
    ...
    <form id="profileForm" method="post" action="/capstone/templates/pharmacy/pharmacy_profile.php" enctype="multipart/form-data" style="margin:0;">
        ...
        <div style="text-align:center;margin-bottom:32px;">
          <div style="position:relative;display:inline-block;">
            <div id="avatarPreview" style="width:120px;height:120px;border-radius:50%;background:linear-gradient(135deg,#0a5d39,#10b981);display:flex;align-items:center;justify-content:center;margin:0 auto 16px;box-shadow:0 8px 24px rgba(10,93,57,0.3);border:4px solid #fff;overflow:hidden;">
              ...
              <?php else: ?>
                <img src="/capstone/assets/img/drug.png" alt="Avatar" style="width:60px;height:60px;object-fit:contain;filter:brightness(0) invert(1);" />
              <?php endif; ?>
            </div>

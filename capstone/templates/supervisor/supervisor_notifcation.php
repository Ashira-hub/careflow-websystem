<?php $page = 'Supervisor Notifications';
      ...
      if (acceptBtn && rejectBtn) {
        if (isNurseRequest) {
          ...
          async function updateSupervisor(status) {
            ...
            if (!res.ok) {
              throw new Error(await res.text().catch(function() {
                return 'Failed to update supervisor notification';
              }));
            }
          }
          async function updateSchedule(status) {
            ...
            if (!r.ok) {
              throw new Error(await r.text().catch(function() {
